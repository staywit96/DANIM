...
<script>
	function Rwrite(){
		location.href="${contextPath}/board/writef.do"
	}
</script>
	...
	<div class="board_wrap">
		...
		<div class="board_list_wrap">
			<div class="board_list">
				<div class="top">
					<div class="RB_UQ" id="FB_UQ">번호</div>
					<div class="QNA_Title" id="FB_Title">제목</div>
					<div class="User_ID " id="User_ID">글쓴이</div>
					<div class="QNA_Date_Created" id="FB_Date_Created">쟉성일</div>
					<div class="cnt" id="FB_View">조회</div>
				</div>
				<c:forEach items="${list}" var="testa">
				<div>
					<div class="RB_UQ" id="FB_UQ">${testa.FB_UQ}</div>
					<div class="QNA_Title" id="FB_Title">
						<a href="${contextPath}/board/viewf.do?FB_UQ=${testa.FB_UQ}">${testa.FB_Title}</a>
					</div>
					<div class="User_ID" id="User_ID"></div>
					<div class="QNA_Date_Created" id="FB_Date_Created">${testa.FB_Date_Created}</div>
					<div class="cnt" id="FB_View">${testa.FB_View}</div>
				</div>
				</c:forEach>
			</div>
			<div class="board_page">
				<ul class="paging">
					<c:if test="${paging.prev}">
						<span><a 
							href='<c:url value="/board/free.do?page=${paging.startPage-1}"/>'>이전</a></span>
								</c:if>
								<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
									var="num">
									<span><a href='<c:url value="/board/free.do?page=${num}"/>'>${num}</a></span>
								</c:forEach>
								<c:if test="${paging.next && paging.endPage>0}">
						<span><a
							href='<c:url value="/board/free.do?page=${paging.endPage+1}"/>'>다음</a></span>
					</c:if>
				</ul>
			</div>
